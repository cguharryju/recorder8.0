  ...
  with tf.Session() as sess:
    ...
    for wav_path in glob.glob(wav_dir + '/*.wav'):
      ...
      for node_id in top_k:
        ...
        docList.append(human_string)
        #print('%s (score = %.5f)' % (human_string, score))
      #print(docList)
      #print(wav_dir.split('/')[-1])

      if wav_dir.split('/')[-1] in docList:
          theWavOK.append(wav_path)
          docList.clear()
  return theWavOK
...
def label_wav(wav_dir, labels, graph, input_name, output_name, how_many_labels):
  ...
  load_graph(graph)
  wavOK=[]
  wavOK.extend(run_graph(wav_dir, labels_list, input_name, output_name, how_many_labels))
  return wavOK
